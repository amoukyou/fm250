...
import { getWrongQuestionIds, addAnswerRecord, toggleFavorite, isFavorite } from '../store/storage'

export default function CategoryWrongPage() {
  ...
  const questions = useMemo(() => {
    ...
    return catQs.filter((q) => wrongIds.has(q.id))
  }, [bankId, categoryId])
  ...
  if (questions.length === 0) return <div className="text-center py-10 text-gray-500">本分類沒有錯題</div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-between items-center">
        <div className="text-sm text-gray-500">{bank?.bankName} · 錯題復習</div>
        <div className="text-sm font-medium text-red-600">{currentIndex + 1} / {questions.length}</div>
      </div>
      <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
        <div className="h-full bg-red-500 rounded-full transition-all" style={{ width: `${((currentIndex + 1) / questions.length) * 100}%` }} />
      </div>
