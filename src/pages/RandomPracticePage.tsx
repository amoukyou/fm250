...
import { addAnswerRecord, toggleFavorite, isFavorite } from '../store/storage'
...
export default function RandomPracticePage() {
  ...
  if (questions.length === 0) {
    return <div className="text-center py-10 text-gray-500">沒有題目</div>
  }
  ...
  function handleSelectMC(option: string) {
    ...
    setShowResult(true)
    const correct = option === (q as SingleChoiceQuestion).answer
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
        <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${((currentIndex + 1) / questions.length) * 100}%` }} />
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-5">
        ...
        {q.type === 'single_choice' ? (
          <>
            <div className="space-y-2">
              {q.options.map((opt, i) => {
                ...
                if (showResult) {
                  if (opt === q.answer) cls += 'bg-green-50 border-green-400 text-green-800 '
                  else if (opt === selectedOption) cls += 'bg-red-50 border-red-400 text-red-800 '
                  else cls += 'bg-gray-50 border-gray-200 text-gray-400 '
                } else {
                  cls += 'bg-white border-gray-200 hover:border-blue-400 hover:bg-blue-50 '
                }
                return (
                  <button key={i} className={cls} onClick={() => handleSelectMC(opt)}>
            ...
            {showResult && q.explanation && (
              <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800">
                <div className="font-medium mb-1">解析</div>{q.explanation}
              </div>
            )}
          </>
        ) : (
          <>
            {!showAnswer ? (
              <button onClick={() => setShowAnswer(true)} className="w-full py-3 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 transition">
                顯示參考答案
              </button>
            ) : (
              <div className="space-y-4">
                <div className="bg-gray-50 border rounded-lg p-4 text-gray-800 whitespace-pre-line">
                  <div className="font-medium text-sm text-gray-500 mb-2">參考答案</div>
